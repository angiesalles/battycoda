...
**Nginx config is generated by `setup_nginx.sh`** — never edit `/etc/nginx/sites-available/battycoda.conf` directly. Edit the script, then run `sudo ./setup_nginx.sh` to regenerate and reload.
