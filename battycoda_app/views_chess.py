...
def chess_proxy_view(request, path=""):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            response = requests.post(target_url, data=request.body, headers=headers, stream=True, timeout=30)
        elif request.method == "PUT":
            response = requests.put(target_url, data=request.body, headers=headers, stream=True, timeout=30)
        elif request.method == "DELETE":
            response = requests.delete(target_url, headers=headers, timeout=30)
        else:
            # For other methods, just pass through
            response = requests.request(
                request.method, target_url, data=request.body, headers=headers, stream=True, timeout=30
            )
