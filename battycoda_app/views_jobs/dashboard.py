...
def jobs_dashboard_view(request):
    ...
    if profile.group:
        ...
        if profile.is_current_group_admin:
            training_jobs = ClassifierTrainingJob.objects.filter(group=profile.group).order_by("-created_at")[:20]
        else:
            training_jobs = ClassifierTrainingJob.objects.filter(created_by=request.user).order_by("-created_at")[:20]
        ...
        if profile.is_current_group_admin:
            clustering_runs = ClusteringRun.objects.filter(
                group=profile.group, status__in=["pending", "running", "completed", "failed"]
            ).order_by("-created_at")[:20]
        else:
            clustering_runs = ClusteringRun.objects.filter(
                created_by=request.user, status__in=["pending", "running", "completed", "failed"]
            ).order_by("-created_at")[:20]
        ...
        if profile.is_current_group_admin:
            spectrogram_jobs = SpectrogramJob.objects.filter(recording__group=profile.group).order_by("-created_at")[
                :20
            ]
        else:
            spectrogram_jobs = SpectrogramJob.objects.filter(created_by=request.user).order_by("-created_at")[:20]
