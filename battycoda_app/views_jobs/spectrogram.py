...
def create_spectrogram_job_view(request, recording_id):
    ...
    if existing_job:
        return JsonResponse(
            {"success": False, "error": "Spectrogram generation already in progress for this recording"}
        )
    ...
    try:
        ...
        return JsonResponse({"success": False, "error": f"Failed to start spectrogram generation: {str(e)}"})
