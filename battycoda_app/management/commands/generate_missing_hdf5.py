...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--batch-size", type=int, default=None, help="Maximum number of recordings to process (default: all)"
        )
        parser.add_argument(
            "--sleep-multiplier",
            type=float,
        ...
        for i, recording in enumerate(missing_hdf5, 1):
            try:
                ...
                if not recording.wav_file or not os.path.exists(recording.wav_file.path):
                    self.stdout.write(self.style.ERROR("  Skipping - WAV file not found"))
                    errors += 1
                    continue
