...
def upload_pickle_segmentation(request, recording_id):
    ...
    try:
        ...
        if recording.group != user.profile.group and recording.created_by != user:
            return JsonResponse(
                {"success": False, "error": "Permission denied: Cannot modify this recording"}, status=403
            )

        # Get the pickle file
        pickle_file = request.FILES.get("pickle_file")
        if not pickle_file:
            return JsonResponse({"success": False, "error": "Missing required field: pickle_file"}, status=400)
        ...
        with transaction.atomic():
            try:
                ...
                return JsonResponse(
                    {"success": False, "error": f"Pickle file processing failed: {str(pickle_error)}"}, status=400
                )
