...
from ..models import Recording, Segmentation, SegmentationAlgorithm
...
def simple_segmentation_algorithms(request):
    ...
    if user_group and user.profile.is_current_group_admin:
        ...
        algorithms = (
            SegmentationAlgorithm.objects.filter(is_active=True)
            .filter(Q(group__isnull=True) | Q(group=user_group))
            .order_by("name")
        )
    else:
    ...
    return JsonResponse({"success": True, "algorithms": algorithms_data, "count": len(algorithms_data)})
...
def simple_start_segmentation(request, recording_id):
    ...
    try:
        ...
        return JsonResponse({"success": False, "error": f"Recording with ID {recording_id} not found"}, status=404)

    # Check permissions
    user_group = user.profile.group
    if recording.created_by != user and (not user_group or recording.group != user_group):
        return JsonResponse(
            {"success": False, "error": "You do not have permission to segment this recording"}, status=403
        )
    ...
    if algorithm_id:
        try:
            ...
            if user_group and user.profile.is_current_group_admin:
                algorithm = (
                    SegmentationAlgorithm.objects.filter(id=algorithm_id, is_active=True)
                    .filter(Q(group__isnull=True) | Q(group=user_group))
                    .first()
                )
            else:
            ...
            if not algorithm:
                return JsonResponse(
                    {"success": False, "error": f"Algorithm with ID {algorithm_id} not found or not accessible"},
                    status=400,
                )
        except ValueError:
            return JsonResponse({"success": False, "error": "Invalid algorithm_id format"}, status=400)
    else:
        ...
        if user_group and user.profile.is_current_group_admin:
            ...
            algorithm = (
                SegmentationAlgorithm.objects.filter(is_active=True, group__isnull=True).order_by("name").first()
            )

        if not algorithm:
            return JsonResponse({"success": False, "error": "No segmentation algorithms available"}, status=400)
    ...
    try:
        ...
        return JsonResponse({"success": False, "error": f"Failed to start segmentation: {str(e)}"}, status=500)
