    ...
    try:
        ...
        try:
            ...
            if user_group:
                ...
                species = Species.objects.get(Q(id=species_id) & (Q(group=user_group) | Q(group__isnull=True)))
            else:
                # User has no group - can access their own species AND system species
                species = Species.objects.get(Q(id=species_id) & (Q(created_by=user) | Q(group__isnull=True)))
        ...
        try:
            ...
            if pickle_file:
                try:
                    ...
                    segmentation_info = {"error": f"Pickle file processing failed: {str(pickle_error)}"}
