...
def simple_species_list(request):
    ...
    if user_group:
        ...
        species = Species.objects.filter(Q(group=user_group) | Q(group__isnull=True))
    else:
        # Fallback to user's own species and system species
        species = Species.objects.filter(Q(created_by=user) | Q(group__isnull=True))
...
def simple_projects_list(request):
    ...
    return JsonResponse({"success": True, "projects": projects_data, "count": len(projects_data)})
...
def simple_recordings_list(request):
    ...
    return JsonResponse({"success": True, "recordings": recordings_data, "count": len(recordings_data)})
