...
def simple_classifiers_list(request):
    ...
    if user_group:
        classifiers = (
            Classifier.objects.filter(Q(group=user_group) | Q(group__isnull=True))
            .filter(is_active=True)
            .order_by("name")
        )
    else:
        classifiers = (
            Classifier.objects.filter(Q(created_by=user) | Q(group__isnull=True))
            .filter(is_active=True)
            .order_by("name")
        )
    ...
    return JsonResponse({"success": True, "classifiers": classifiers_data, "count": len(classifiers_data)})
...
def simple_start_classification(request, segmentation_id):
    ...
    if not user.profile.group:
        return JsonResponse(
            {"success": False, "error": "User must be assigned to a group to start classification"}, status=400
        )
    ...
    try:
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "error": f"Segmentation with ID {segmentation_id} not found"}, status=404
            )
        ...
        if recording.created_by != user and (not user_group or recording.group != user_group):
            return JsonResponse(
                {"success": False, "error": "You do not have permission to classify this segmentation"}, status=403
            )
        ...
        if not classifier_id:
            return JsonResponse({"success": False, "error": "classifier_id is required"}, status=400)
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "error": f"Classifier with ID {classifier_id} not found or not accessible"},
                status=400,
            )
        ...
        return JsonResponse({"success": False, "error": f"Failed to start classification: {str(e)}"}, status=500)
...
def simple_classification_runs_list(request):
    ...
    if recording_id:
        try:
            ...
            return JsonResponse({"success": False, "error": "Invalid recording_id format"}, status=400)
        ...
        runs_data.append(
            {
                ...
                "project_name": (
                    run.segmentation.recording.project.name
                    if run.segmentation and run.segmentation.recording.project
                    else None
                ),
                "project_id": (
                    run.segmentation.recording.project.id
                    if run.segmentation and run.segmentation.recording.project
                    else None
                ),
    ...
    return JsonResponse({"success": True, "classification_runs": runs_data, "count": len(runs_data)})
