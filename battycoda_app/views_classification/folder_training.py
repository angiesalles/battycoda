...
def validate_training_folder(folder_path, species):
    ...
    if stats['unlabeled_files']:
        errors.append(f"Found {len(stats['unlabeled_files'])} files without proper labels (must be in format: NUMBER_LABEL.wav)")
    ...
    if species_calls.exists():
        ...
        if missing_in_folder:
            warnings.append(f"Species has call types not present in training data: {', '.join(sorted(missing_in_folder))}")

        extra_in_folder = folder_labels - species_call_names
        if extra_in_folder:
            errors.append(f"Training data contains call types not defined for this species: {', '.join(sorted(extra_in_folder))}")

    min_samples_per_class = 2
    insufficient_classes = [label for label, count in stats['files_by_label'].items() if count < min_samples_per_class]
    if insufficient_classes:
        errors.append(f"Some call types have fewer than {min_samples_per_class} samples: {', '.join(insufficient_classes)}")
...
def create_classifier_from_folder_view(request, folder_name=None):
    ...
    if request.method == "POST":
        ...
        parameters = {
            ...
            "training_data_folder": folder_path
        }
