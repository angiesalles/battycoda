#' Create a new PaginatedSpeciesList
#'
#' @description
#' PaginatedSpeciesList Class
#'
#' @docType class
#' @title PaginatedSpeciesList
#' @description PaginatedSpeciesList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{Species})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedSpeciesList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedSpeciesList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedSpeciesList as a base R list.
    #' @examples
    #' # convert array of PaginatedSpeciesList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedSpeciesList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedSpeciesListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedSpeciesListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedSpeciesListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedSpeciesListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedSpeciesListObject[["results"]] <-
          lapply(self$`results`, function(x) x$toSimpleType())
      }
      return(PaginatedSpeciesListObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PaginatedSpeciesList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedSpeciesList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Species]", loadNamespace("openapi"))
      }
      self
    },
    ...
    #' @return PaginatedSpeciesList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedSpeciesList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedSpeciesList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedSpeciesList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedSpeciesList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedSpeciesList$unlock()
#
## Below is an example to define the print function
# PaginatedSpeciesList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedSpeciesList$lock()
