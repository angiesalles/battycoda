#' Create a new PaginatedRecordingList
#'
#' @description
#' PaginatedRecordingList Class
#'
#' @docType class
#' @title PaginatedRecordingList
#' @description PaginatedRecordingList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{Recording})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedRecordingList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedRecordingList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedRecordingList as a base R list.
    #' @examples
    #' # convert array of PaginatedRecordingList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedRecordingList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedRecordingListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedRecordingListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedRecordingListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedRecordingListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedRecordingListObject[["results"]] <-
          lapply(self$`results`, function(x) x$toSimpleType())
      }
      return(PaginatedRecordingListObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PaginatedRecordingList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedRecordingList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Recording]", loadNamespace("openapi"))
      }
      self
    },
    ...
    #' @return PaginatedRecordingList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedRecordingList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedRecordingList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedRecordingList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedRecordingList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedRecordingList$unlock()
#
## Below is an example to define the print function
# PaginatedRecordingList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedRecordingList$lock()
