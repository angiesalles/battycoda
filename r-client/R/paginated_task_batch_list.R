#' Create a new PaginatedTaskBatchList
#'
#' @description
#' PaginatedTaskBatchList Class
#'
#' @docType class
#' @title PaginatedTaskBatchList
#' @description PaginatedTaskBatchList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{TaskBatch})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedTaskBatchList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedTaskBatchList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedTaskBatchList as a base R list.
    #' @examples
    #' # convert array of PaginatedTaskBatchList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedTaskBatchList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedTaskBatchListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedTaskBatchListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedTaskBatchListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedTaskBatchListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedTaskBatchListObject[["results"]] <-
          lapply(self$`results`, function(x) x$toSimpleType())
      }
      return(PaginatedTaskBatchListObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PaginatedTaskBatchList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedTaskBatchList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[TaskBatch]", loadNamespace("openapi"))
      }
      self
    },
    ...
    #' @return PaginatedTaskBatchList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedTaskBatchList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedTaskBatchList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedTaskBatchList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedTaskBatchList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedTaskBatchList$unlock()
#
## Below is an example to define the print function
# PaginatedTaskBatchList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedTaskBatchList$lock()
