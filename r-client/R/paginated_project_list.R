#' Create a new PaginatedProjectList
#'
#' @description
#' PaginatedProjectList Class
#'
#' @docType class
#' @title PaginatedProjectList
#' @description PaginatedProjectList Class
#' @format An \code{R6Class} generator object
#' @field count  integer
#' @field next  character [optional]
#' @field previous  character [optional]
#' @field results  list(\link{Project})
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
PaginatedProjectList <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PaginatedProjectList class.
    #'
    #' @param count count
    ...
    #'
    #' @return PaginatedProjectList as a base R list.
    #' @examples
    #' # convert array of PaginatedProjectList (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PaginatedProjectList to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PaginatedProjectListObject <- list()
      if (!is.null(self$`count`)) {
        PaginatedProjectListObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`next`)) {
        PaginatedProjectListObject[["next"]] <-
          self$`next`
      }
      if (!is.null(self$`previous`)) {
        PaginatedProjectListObject[["previous"]] <-
          self$`previous`
      }
      if (!is.null(self$`results`)) {
        PaginatedProjectListObject[["results"]] <-
          lapply(self$`results`, function(x) x$toSimpleType())
      }
      return(PaginatedProjectListObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PaginatedProjectList
    #'
    #' @param input_json the JSON input
    #' @return the instance of PaginatedProjectList
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Project]", loadNamespace("openapi"))
      }
      self
    },
    ...
    #' @return PaginatedProjectList in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PaginatedProjectList and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`count`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedProjectList: the required field `count` is missing."))
      }
      ...
      if (!is.null(input_json$`results`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PaginatedProjectList: the required field `results` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PaginatedProjectList
    toString = function() {
      self$toJSONString()
    },
...
# PaginatedProjectList$unlock()
#
## Below is an example to define the print function
# PaginatedProjectList$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PaginatedProjectList$lock()
