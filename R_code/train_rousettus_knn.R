#!/usr/bin/env Rscript
#
# Train a new KNN model for Rousettus using existing training data
#
...
output_model_path <- "/app/data/models/rousettus_knn_model.RData"
test_split <- 0.2

# Print configuration
cat("=== Training new Rousettus KNN model ===\n")
...
result <- tryCatch({
  train_model(data_folder, output_model_path, test_split, "knn")
}, error = function(e) {
  cat("ERROR in KNN training:", e$message, "\n")
  return(list(
    status = "error",
...
if (result$status == "success") {
  ...
  if (!is.null(result$best_hyperparameters)) {
    cat("Best hyperparameters - k: ", result$best_hyperparameters$k,
        ", distance: ", result$best_hyperparameters$distance, "\n")
  }
} else {
