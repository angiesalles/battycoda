...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (usernameField) {
        ...
        usernameField.addEventListener('input', function(e) {
            ...
            if (!regex.test(this.value)) {
                ...
                usernameWarningElement.textContent = 'Username can only contain letters, numbers, and the characters ._-';
            }
            
            // Start timer to check if username exists (wait for user to finish typing)
            if (this.value.length > 0) {
                usernameTypingTimer = setTimeout(() => checkUsername(usernameCheckURL, csrfToken), doneTypingInterval);
            } else {
    ...
    if (emailField) {
        ...
        emailField.addEventListener('input', function(e) {
            ...
            if (this.value.length > 0) {
                emailTypingTimer = setTimeout(() => checkEmail(emailCheckURL, csrfToken), doneTypingInterval);
            } else {
