...
(function(ClusterMapping) {
     ...
     */
    ClusterMapping.initializeClusterPreviewModal = function(createMapping, getSelectedClusterId) {
        $('#clusterPreviewModal').on('show.bs.modal', function() {
            ...
            var selectElement = $('#mapping-call-select');
            selectElement.empty();
            selectElement.append('<option value="">Select a call type to map to...</option>');

            $('.call-box').each(function() {
                var callId = $(this).find('.mapping-container').data('call-id');
                var callName = $(this).find('h6').text().trim();
                var speciesName = $(this).closest('.species-section').find('h5').text().trim();
        ...
        $('#create-mapping-btn').on('click', function() {
            var callId = $('#mapping-call-select').val();
            var selectedClusterId = getSelectedClusterId();
            ...
            var modal = bootstrap.Modal.getInstance(document.getElementById('clusterPreviewModal'));
            if (modal) modal.hide();
        });
    };
     ...
     */
    ClusterMapping.loadClusterDetails = function(clusterId) {
        ...
        var sizeText = clusterBox.find('.text-muted:contains("Size")').text();
        var coherenceText = clusterBox.find('.text-muted:contains("Coherence")').text();
