  ...
  function loadAudio() {
    ...
    audioPlayer.addEventListener('error', function (e) {
      ...
      errorDiv.textContent =
        'Failed to load audio snippet. Error: ' +
        (audioPlayer.error
          ? 'Code: ' +
            audioPlayer.error.code +
            ', Message: ' +
            getErrorMessage(audioPlayer.error.code)
          : 'Unknown error');
      container.appendChild(errorDiv);
    });
    ...
    audioPlayerLong.addEventListener('error', function (e) {
      ...
      errorDiv.className = 'alert alert-danger mt-2 audio-error-message';
      errorDiv.textContent =
        'Failed to load long audio snippet. Error: ' +
        (audioPlayerLong.error
          ? 'Code: ' +
            audioPlayerLong.error.code +
            ', Message: ' +
            getErrorMessage(audioPlayerLong.error.code)
          : 'Unknown error');
      container.appendChild(errorDiv);
    });
    ...
    function getErrorMessage(code) {
      switch (code) {
        case 1:
          return 'MEDIA_ERR_ABORTED - The fetching process was aborted by the user.';
        case 2:
          return 'MEDIA_ERR_NETWORK - A network error occurred while fetching the media.';
        case 3:
          return 'MEDIA_ERR_DECODE - A decoding error occurred.';
        case 4:
