...
export class UIState {
  ...
  updateTimeDisplay() {
    ...
    if (this.player.zoomLevel > 1 && this.player.progressContainer) {
      ...
      const startPercent =
        ((this.player.zoomOffset * this.player.duration) / this.player.duration) * 100;
  ...
  updateSelectionDisplay() {
    ...
    if (this.player.selectionStart !== null && this.player.selectionEnd !== null) {
      ...
      this.player.selectionRangeEl.textContent = `Selected: ${this.player.selectionStart.toFixed(2)}s - ${this.player.selectionEnd.toFixed(2)}s (${duration.toFixed(2)}s)`;
    } else if (this.player.selectionStart !== null) {
      this.player.selectionRangeEl.textContent = `Start: ${this.player.selectionStart.toFixed(2)}s (select end point)`;
    } else {
